...
const Description = () => {
    const {id} = useParams()
    const {getCharacter, singleCharacter, addToFavourite} = CharConsumer()
    const [setHeartRed, setSetHeartRed] = React.useState(false)
            ...
            <h2 className='text-center text-capitalize my-5 pb-4'>{singleCharacter.name}</h2>
              <div className="desc-wrapper">
                ...
                <div className="desc-info-container">
                   ...
                   <span>
                      <AiOutlineHeart
                         onClick={()=> handleFavourite(singleCharacter.name)}
                          className={setHeartRed ? 'svg-active' : ''}
                       />
                   </span>
                </div>
